...
import zx.soft.pca.utils.PCALog;
import zx.soft.pca.utils.PCALog.LogType;
import zx.soft.pca.utils.TimeCounter;
...
public class SimplePCA {
	...
	public SimplePCA(int numComponents, DoubleMatrix2D reducedData, DoubleMatrix1D eigenValues, DoubleMatrix1D mean) {
	...
	public SimplePCA(DoubleMatrix2D data) {
		this.data = data;
	}
	...
	public void computePCA(int numComponents) {
		...
		this.numComponents = numComponents;
		PCALog.info(LogType.MODEL, "PCA computation with " + numComponents + " dimensions.");
		centerData();
		doComputeBasis();
	...
	private void computeMean() {
		PCALog.debug(LogType.MODEL, "PCA: compute mean sample.");
	...
	private void centerData() {
		PCALog.debug(LogType.MODEL, "PCA: lock data.");
		this.dataLock = true;
		ensureMean();
